(function($){var defaluts={
	...
	$.fn.extend({"thumbnailImg":function(options){
		...
		var l=$(opts.small_elem).find("ul li").length;var l_mean;
		if(l<opts.vis){l_mean=0}
		else{
	...
	$(opts.left_btn).click(function(){
		var i;$(opts.small_elem).find("ul li").each(
			function(index){
				if($(this).hasClass(opts.state)){
					i=index
				}
			});
		i--;if(i<0){i=l-1}t=i;Img(i)
	});
	...
	$(opts.right_btn).click(
		function(){
			var i;$(opts.small_elem).find("ul li").each(
				  function(index){
					if($(this).hasClass(opts.state)){i=index}
				});
			i++;if(i>l-1){i=0}t=i;Img(i)
	});
